...
void MagnetometerROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->magneticFieldWorld.X() = this->parameters.intensity * cos(this->parameters.inclination) * cos(this->parameters.heading - this->parameters.declination);
  this->magneticFieldWorld.Y() = this->parameters.intensity * cos(this->parameters.inclination) * sin(this->parameters.heading - this->parameters.declination);
  this->magneticFieldWorld.Z() = this->parameters.intensity * -1 * sin(this->parameters.inclination);

  this->AddNoiseModel("turn_on_bias", this->parameters.turnOnBias);

  // Add Gaussian noise to the model
  this->turnOnBias.X() = this->GetGaussianNoise("turn_on_bias", this->noiseAmp);
  this->turnOnBias.Y() = this->GetGaussianNoise("turn_on_bias", this->noiseAmp);
  this->turnOnBias.Z() = this->GetGaussianNoise("turn_on_bias", this->noiseAmp);
  ...
  this->rosMsg.magnetic_field_covariance[0] = this->parameters.noiseXY * this->parameters.noiseXY;
  this->rosMsg.magnetic_field_covariance[4] = this->parameters.noiseXY * this->parameters.noiseXY;
  this->rosMsg.magnetic_field_covariance[8] = this->parameters.noiseZ * this->parameters.noiseZ;

  // Initialize the default magnetometer output
  this->rosSensorOutputPub = this->rosNode->advertise<sensor_msgs::MagneticField>(this->sensorOutputTopic, 1);
}
...
bool MagnetometerROSPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  ignition::math::Vector3d noise(this->GetGaussianNoise("noise_xy", this->noiseAmp), this->GetGaussianNoise("noise_xy", this->noiseAmp), this->GetGaussianNoise("noise_z", this->noiseAmp));
  this->measMagneticField = pose.Rot().RotateVectorReverse(this->magneticFieldWorld) + this->turnOnBias + noise;
