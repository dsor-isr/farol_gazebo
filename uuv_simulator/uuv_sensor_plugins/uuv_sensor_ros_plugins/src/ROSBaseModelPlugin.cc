...
ROSBaseModelPlugin::ROSBaseModelPlugin() {
  ...
  this->localNEDFrame.Rot() = ignition::math::Quaterniond(ignition::math::Vector3d(M_PI, 0, 0));
  
  // Initialize the local NED frame
  this->tfLocalNEDFrame.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
  this->tfLocalNEDFrame.setRotation(tf::createQuaternionFromRPY(M_PI, 0.0, 0.0));
...
ROSBaseModelPlugin::~ROSBaseModelPlugin() {
  if (this->tfBroadcaster) delete this->tfBroadcaster;
}
...
void ROSBaseModelPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  GetSDFParam<bool>(_sdf, "enable_local_ned_frame", this->enableLocalNEDFrame, true);
  ...
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&ROSBasePlugin::OnUpdate, this, _1));
}
