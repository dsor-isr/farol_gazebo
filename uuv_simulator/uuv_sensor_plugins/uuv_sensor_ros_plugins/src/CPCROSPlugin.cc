...
void CPCROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  GZ_ASSERT(this->smoothingLength > 0, "Radius of the sensor must be greater than zero");
  ...
  GZ_ASSERT(salinityUnit.compare("ppt") == 0 || salinityUnit.compare("ppm") == 0 || salinityUnit.compare("psu") == 0, "Invalid salinity unit, it can be ppt, ppm or psu");
  ...
  GZ_ASSERT(this->plumeSalinityValue >= 0.0, "Plume salinity value must be greater or equal to zero");
  GZ_ASSERT(this->plumeSalinityValue < this->waterSalinityValue, "Plume salinity value must be lower than the water salinity value");

  this->particlesSub = this->rosNode->subscribe<sensor_msgs::PointCloud>(inputTopic, 1, boost::bind(&CPCROSPlugin::OnPlumeParticlesUpdate, this, _1));
  ...
  this->rosSensorOutputPub = this->rosNode->advertise<uuv_sensor_ros_plugins_msgs::ChemicalParticleConcentration>(this->sensorOutputTopic, 1);
  this->salinityPub = this->rosNode->advertise<uuv_sensor_ros_plugins_msgs::Salinity>(salinityTopic, 1);
...
bool CPCROSPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  if (!this->EnableMeasurement(_info) || this->updatingCloud) return false;
  ...
  this->salinityMsg.salinity = this->waterSalinityValue * (1 - std::min(1.0, this->outputMsg.concentration)) + std::min(1.0, this->outputMsg.concentration) * this->plumeSalinityValue;
...
void CPCROSPlugin::OnPlumeParticlesUpdate(const sensor_msgs::PointCloud::ConstPtr &_msg) {
  ...
  if (this->rosSensorOutputPub.getNumSubscribers() > 0) {
    ...
    ignition::math::Vector3d scVec = this->link->GetWorld()->SphericalCoords()->SphericalFromLocal(cartVec);
    ...
    for (int i = 0; i < _msg->points.size(); i++) {
      particle = ignition::math::Vector3d(_msg->points[i].x, _msg->points[i].y, _msg->points[i].z);
      smoothingParam = std::pow(initSmoothingLength + this->gamma * (currentTime - _msg->channels[0].values[i]), 1.5);
      distToParticle = linkPos.Distance(particle);

      // Compute particle concentration
      if (distToParticle >= 0 && distToParticle < smoothingParam)
        particleConc = 4.0 - 6.0 * std::pow(distToParticle / smoothingParam, 2) + 3.0 * std::pow(distToParticle / smoothingParam, 3);
