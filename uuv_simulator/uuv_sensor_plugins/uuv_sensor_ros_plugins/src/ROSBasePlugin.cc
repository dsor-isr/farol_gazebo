...
ROSBasePlugin::~ROSBasePlugin() {
  ...
  // Shutdown ROS and the plugin connnection
  if (this->rosNode) this->rosNode->shutdown();
...
bool ROSBasePlugin::InitBasePlugin(sdf::ElementPtr _sdf) {
  ...
  if (!sensorTopic.empty()) this->sensorOutputTopic = sensorTopic;
  GZ_ASSERT(!this->sensorOutputTopic.empty(), "Sensor output topic has not been provided");
  ...
  if (!ros::isInitialized()) {
    gzerr << "Not loading sensor plugin since ROS has not been properly " << "initialized." << std::endl;
    return false;
  }
  ...
  if (_sdf->HasElement("static_reference_frame")) {
    ...
    GetSDFParam<std::string>(_sdf, "static_reference_frame", this->referenceFrameID, "world");
    ...
    if (this->referenceFrameID.compare("world") != 0) {
      this->tfStaticSub = this->rosNode->subscribe<tf::tfMessage>("/tf_static", 1, boost::bind(&ROSBasePlugin::GetTFMessage, this, _1));
    } else {
      this->isReferenceInit = true;
    }
  } else if (_sdf->HasElement("reference_link_name")) {
    GZ_ASSERT(this->referenceLink != NULL, "Reference link has not been initialized");
  } else {
    this->referenceFrameID = "world";
    this->referenceLink = NULL;
    this->isReferenceInit = true;
  ...
  this->changeSensorSrv = this->rosNode->advertiseService(this->sensorOutputTopic + "/change_state", &ROSBasePlugin::ChangeSensorState, this);

  this->pluginStatePub = this->rosNode->advertise<std_msgs::Bool>(this->sensorOutputTopic + "/state", 1);

  GetSDFParam<double>(_sdf, "noise_sigma", this->noiseSigma, 0.0);
  GZ_ASSERT(this->noiseSigma >= 0.0, "Signal noise sigma must be greater or equal to zero");

  GetSDFParam<double>(_sdf, "noise_amplitude", this->noiseAmp, 0.0);
  GZ_ASSERT(this->noiseAmp >= 0.0, "Signal noise amplitude must be greater or equal to zero");
...
void ROSBasePlugin::GetTFMessage(const tf::tfMessage::ConstPtr &_msg) {
  ...
  // Use lazy initialization - once a TF is published with the reference frame with respect to the world
  // we initialize the reference and save it. We then assume that the sensor is fixed, so once the initialization
  // is done, we can just return 
  if (this->isReferenceInit) return;

  if (_msg->transforms.size() > 0) {
    for (auto t : _msg->transforms) {
      if (!t.header.frame_id.compare("world") && !t.child_frame_id.compare(this->referenceFrameID)) {
        ...
        // Get the orientation of the reference frame
...
bool ROSBasePlugin::ChangeSensorState(uuv_sensor_ros_plugins_msgs::ChangeSensorState::Request& _req, uuv_sensor_ros_plugins_msgs::ChangeSensorState::Response& _res) {
  
  this->isOn.data = _req.on;
  _res.success = true;
  std::string message = this->sensorOutputTopic + "::";

  message += (_req.on) ? " ON" : " OFF";
...
void ROSBasePlugin::PublishState() {
  ...
  // Publish the boolean whether the sensor is on or not
  this->pluginStatePub.publish(this->isOn);
}

double ROSBasePlugin::GetGaussianNoise(double _amp) {

  // return the gaussian noise affected by the amplitude
...
bool ROSBasePlugin::AddNoiseModel(std::string _name, double _sigma) {
  ...
  if (this->noiseModels.count(_name)) return false;
