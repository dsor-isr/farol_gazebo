...
void CustomBatteryConsumerROSPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("topic_device_state")) {
    ...
    if (!topicName.empty()) {
      this->deviceStateSub = this->rosNode->subscribe<std_msgs::Bool>(topicName, 1, boost::bind(&CustomBatteryConsumerROSPlugin::UpdateDeviceState, this, _1));
    }
  } else {
...
void CustomBatteryConsumerROSPlugin::UpdateDeviceState(const std_msgs::Bool::ConstPtr &_msg) {
