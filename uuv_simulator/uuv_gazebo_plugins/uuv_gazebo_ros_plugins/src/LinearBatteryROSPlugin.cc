...
void LinearBatteryROSPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (updateRate <= 0.0) {
    gzmsg << "Invalid update rate, setting it to 2 Hz, rate=" << updateRate << std::endl;
    updateRate = 2;
  }

  this->rosNode.reset(new ros::NodeHandle(this->robotNamespace));
  this->batteryStatePub = this->rosNode->advertise<sensor_msgs::BatteryState>("battery_state", 0);

  this->updateTimer = this->rosNode->createTimer(ros::Duration(1 / updateRate), boost::bind(&LinearBatteryROSPlugin::PublishBatteryState, this));
