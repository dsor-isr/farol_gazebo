...
void JointStatePublisher::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  this->node = boost::shared_ptr<ros::NodeHandle>(new ros::NodeHandle(this->robotNamespace));
  ...
  this->jointStatePub = this->node->advertise<sensor_msgs::JointState>(this->robotNamespace + "/joint_states", 1);
  this->lastUpdate = this->world->SimTime();
  // Connect the update function to the Gazebo callback
  this->updateConnection = gazebo::event::Events::ConnectWorldUpdateBegin(boost::bind(&JointStatePublisher::OnUpdate, this, _1));
}
...
bool JointStatePublisher::IsIgnoredJoint(std::string _jointName) {
  ...
  for (auto joint : this->movingJoints) {
    if (_jointName.compare(joint) == 0) return false;
  }
  return true;
}
}
