...
UmbilicalModel* UmbilicalModelFactory::CreateUmbilicalModel(sdf::ElementPtr _sdf, physics::ModelPtr _model) {
    
  std::cout << __PRETTY_FUNCTION__ << std::endl;
  if (!_sdf->HasElement("type")) {
    std::cerr << "umbilical_model does not have an type element" << std::endl;
    return NULL;
  }

  std::string identifier = _sdf->Get<std::string>("type");
  if (creators_.find(identifier) == creators_.end()) {
    std::cerr << "Cannot create UmbilicalModel with unknown identifier: " << identifier << std::endl;
    return NULL;
  }
...
bool UmbilicalModelFactory::RegisterCreator(const std::string& _identifier, UmbilicalModelCreator _creator) {
  
  if (creators_.find(_identifier) != creators_.end()) {
    std::cerr << "Warning: Registering UmbilicalModel with identifier: " << _identifier << " twice" << std::endl;
  }
...
UmbilicalModelBerg::UmbilicalModelBerg(sdf::ElementPtr _sdf, physics::ModelPtr _model) {
  
  GZ_ASSERT(_sdf->HasElement("connector_link"), "Could not find connector_link.");
...
UmbilicalModel* UmbilicalModelBerg::create(sdf::ElementPtr _sdf, physics::ModelPtr _model) {
  std::cout << __PRETTY_FUNCTION__ << std::endl;
  return new UmbilicalModelBerg(_sdf, _model);
}


void UmbilicalModelBerg::OnUpdate(const common::UpdateInfo &_info,  const ignition::math::Vector3d& _flow) {
  ignition::math::Pose3d pose;
  pose = this->connector->WorldPose();

  double h = -pose.Pos().Z();

  // Allow for some wiggle room when the UUV emerges.
  GZ_ASSERT(h < 10.0, "z coordinate should be negative");
