...
void UmbilicalPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  GZ_ASSERT(_sdf->HasElement("umbilical_model"), "Could not find umbilical_model.");
  this->umbilical.reset(UmbilicalModelFactory::GetInstance().CreateUmbilicalModel(_sdf->GetElement("umbilical_model"), _model));
  this->umbilical->Init();
  ...
  GZ_ASSERT(_sdf->HasElement("flow_velocity_topic"), "Umbilical model requires flow velocity topic");
  ...
  this->flowSubscriber = this->node->Subscribe(flowTopic, &UmbilicalPlugin::UpdateFlowVelocity, this);

  // Listen to the update event. This event is broadcast every
  // simulation iteration.
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&UmbilicalPlugin::OnUpdate, this, _1));
}
