...
bool LiftDrag::CheckForElement(sdf::ElementPtr _sdf, const std::string &element) {
  
  if (!_sdf->HasElement(element)) {
    std::cerr << " LiftDrag: Missing required element: " << element << std::endl;
    return false;
  }
  return true;
}
...
LiftDrag* LiftDragFactory::CreateLiftDrag(sdf::ElementPtr _sdf) {
  ...
  if (creators_.find(identifier) == creators_.end()) {
    std::cerr << "Cannot create LiftDrag with unknown identifier: " << identifier << std::endl;
    return NULL;
  }
...
bool LiftDragFactory::RegisterCreator(const std::string& _identifier, LiftDragCreator _creator) {
  
  if (creators_.find(_identifier) != creators_.end()) {
    std::cerr << "Warning: Registering LiftDrag with identifier: " << _identifier << " twice" << std::endl;
  }
...
LiftDrag* LiftDragQuadratic::create(sdf::ElementPtr _sdf) {
  ...
  if (!_sdf->HasElement("lift_constant")) {
    std::cerr << "LiftDragQuadratic: expected element lift_constant" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("drag_constant")) {
    std::cerr << "LiftDragQuadratic: expected element drag_constant" << std::endl;
    return NULL;
  }
  ...
  return new LiftDragQuadratic(_sdf->Get<double>("lift_constant"), _sdf->Get<double>("drag_constant"));
}


ignition::math::Vector3d LiftDragQuadratic::compute(const ignition::math::Vector3d &_velL) {
...
bool LiftDragQuadratic::GetParam(std::string _tag, double& _output) {
  ...
  gzmsg << "LiftDragQuadratic::GetParam <" << _tag << ">=" << _output << std::endl;
  return true;
}
...
ignition::math::Vector3d LiftDragTwoLines::compute(const ignition::math::Vector3d &_velL) {
  ...
  if (alpha > this->alphaStall) {
    ...
    cl = this->cla*this->alphaStall + this->claStall * diffAlpha;
    cd = this->cda*this->alphaStall + this->cdaStall * diffAlpha;
  } else if (alpha < -this->alphaStall) {
    // stall
    double sumAlpha = alpha + this->alphaStall;
    cl = -this->cla * this->alphaStall + this->cdaStall * sumAlpha;
