...
ConversionFunction* ConversionFunctionFactory::CreateConversionFunction(sdf::ElementPtr _sdf) {
  ...
  if (creators_.find(identifier) == creators_.end()) {
    std::cerr << "Cannot creatae ConversionFunction with unknown identifier: " << identifier << std::endl;
    return NULL;
  }
...
bool ConversionFunctionFactory::RegisterCreator(const std::string& _identifier, ConversionFunctionCreator _creator) {
  
  if (creators_.find(_identifier) != creators_.end()) {
    std::cerr << "Warning: Registering ConversionFunction with identifier: " << _identifier << " twice" << std::endl;
  }

  creators_[_identifier] = _creator;
  std::cout << "Registered ConversionFunction type " << _identifier << std::endl;
  return true;
}


const std::string ConversionFunctionBasic::IDENTIFIER = "Basic";
REGISTER_CONVERSIONFUNCTION_CREATOR(ConversionFunctionBasic, &ConversionFunctionBasic::create)


ConversionFunction* ConversionFunctionBasic::create(sdf::ElementPtr _sdf) {
  if (!_sdf->HasElement("rotorConstant")) {
    std::cerr << "ConversionFunctionBasic::Expected element rotorConstant" << std::endl;
    return NULL;
  }
...
bool ConversionFunctionBasic::GetParam(std::string _tag, double& _output) {
  ...
  gzmsg << "ConversionFunctionBasic::GetParam <" << _tag << ">=" << _output << std::endl;
  return true;
}


ConversionFunctionBasic::ConversionFunctionBasic(double _rotorConstant) : rotorConstant(_rotorConstant) {
  gzmsg << "ConversionFunctionBasic::Create conversion function" << std::endl << "\t- rotorConstant: " << this->rotorConstant << std::endl;
}



const std::string ConversionFunctionBessa::IDENTIFIER = "Bessa";
REGISTER_CONVERSIONFUNCTION_CREATOR(ConversionFunctionBessa, &ConversionFunctionBessa::create)


ConversionFunction* ConversionFunctionBessa::create(sdf::ElementPtr _sdf) {
  if (!_sdf->HasElement("rotorConstantL")) {
    std::cerr << "ConversionFunctionBasic: expected element rotorConstantL" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("rotorConstantR")) {
    std::cerr << "ConversionFunctionBasic::Expected element rotorConstantR" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("deltaL")) {
    std::cerr << "ConversionFunctionBasic::Expected element deltaL" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("deltaR")) {
    std::cerr << "ConversionFunctionBasic::Expected element deltaR" << std::endl;
    return NULL;
  }
...
ConversionFunctionBessa::ConversionFunctionBessa(double _rotorConstantL, double _rotorConstantR, double _deltaL, double _deltaR) :
  rotorConstantL(_rotorConstantL),
  rotorConstantR(_rotorConstantR),
  deltaL(_deltaL),
  deltaR(_deltaR) {

  GZ_ASSERT(rotorConstantL >= 0.0, "ConversionFunctionBessa: rotorConstantL should be >= 0");
  GZ_ASSERT(rotorConstantR >= 0.0, "ConversionFunctionBessa: rotorConstantR should be >= 0");
  GZ_ASSERT(deltaL <= 0.0, "ConversionFunctionBessa: deltaL should be <= 0");
  GZ_ASSERT(deltaR >= 0.0, "ConversionFunctionBessa: deltaR should be >= 0");
...
bool ConversionFunctionBessa::GetParam(std::string _tag, double& _output) {
  ...
  gzmsg << "ConversionFunctionBessa::GetParam <" << _tag << ">=" << _output << std::endl;
  return true;
}


const std::string ConversionFunctionLinearInterp::IDENTIFIER = "LinearInterp";
REGISTER_CONVERSIONFUNCTION_CREATOR(ConversionFunctionLinearInterp, &ConversionFunctionLinearInterp::create)


ConversionFunction* ConversionFunctionLinearInterp::create(sdf::ElementPtr _sdf) {
  
  if (!_sdf->HasElement("inputValues")) {
    std::cerr << "ConversionFunctionLinearInterp::Expected element inputValues" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("outputValues")) {
    std::cerr << "ConversionFunctionLinearInterp::Expected element outputValues" << std::endl;
    return NULL;
  }
  ...
  if (in.size() < 1) {
    std::cerr << "ConversionFunctionLinearInterp::" << "Need at least one input/output pair" << std::endl;
    return NULL;
  }

  if (in.size() != out.size()) {
    std::cerr << "ConversionFunctionLinearInterp::" << "Number of input and output values should be the same" << std::endl;
    return NULL;
  }
...
ConversionFunctionLinearInterp::ConversionFunctionLinearInterp(const std::vector<double> &_input, const std::vector<double> &_output) {
  ...
  gzmsg << "ConversionFunctionLinearInterp::Create conversion function" << std::endl;
  gzmsg << "\t- Input values:" << std::endl;
  for (auto& i : lookupTable) std::cout << i.first << " ";
  std::cout << std::endl;
  gzmsg << "\t- Output values:" << std::endl;
  for (auto& i : lookupTable) std::cout << i.second << " ";
  std::cout << std::endl;
}
