...
namespace gazebo
{
  ...
  void ThrusterPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    this->thrusterDynamics.reset(DynamicsFactory::GetInstance().CreateDynamics(_sdf->GetElement("dynamics")));

    // Thrust conversion function
    GZ_ASSERT(_sdf->HasElement("conversion"), "Could not find dynamics");
    this->conversionFunction.reset(ConversionFunctionFactory::GetInstance().CreateConversionFunction(_sdf->GetElement("conversion")));
    ...
    this->thrustTopicPublisher = this->node->Advertise<msgs::Vector3d>(this->topicPrefix + "thrust");

    // Subscribe to the input signal topic
    this->commandSubscriber = this->node->Subscribe(this->topicPrefix + "input", &ThrusterPlugin::UpdateInput, this);

    // Connect the update event
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&ThrusterPlugin::Update, this, _1));
  ...
  void ThrusterPlugin::Update(const common::UpdateInfo &_info)
  {
    ...
    dynamicState = this->propellerEfficiency * this->thrusterDynamics->update(dynamicsInput, _info.simTime.Double());

    GZ_ASSERT(!std::isnan(dynamicState), "Invalid dynamic state");
    // Multiply the output force magnitude with the efficiency
    this->thrustForce = this->thrustEfficiency * this->conversionFunction->convert(dynamicState);
