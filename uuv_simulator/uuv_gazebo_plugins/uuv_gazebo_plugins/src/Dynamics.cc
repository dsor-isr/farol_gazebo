...
Dynamics* DynamicsFactory::CreateDynamics(sdf::ElementPtr _sdf) {
  ...
  if (creators_.find(identifier) == creators_.end()) {
    std::cerr << "Cannot create ThrusterDynamics with unknown identifier: " << identifier << std::endl;
    return NULL;
  }
...
bool DynamicsFactory::RegisterCreator(const std::string& _identifier, DynamicsCreator _creator) {
  
  if (creators_.find(_identifier) != creators_.end()) {
    std::cerr << "Warning: Registering ThrusterDynamics with identifier: " << _identifier << " twice" << std::endl;
  }
...
Dynamics* DynamicsFirstOrder::create(sdf::ElementPtr _sdf) {
  ...
  if (!_sdf->HasElement("timeConstant")) {
    std::cerr << "DynamicsFirstOrder: expected element time_constant" << std::endl;
    return NULL;
  }
...
DynamicsFirstOrder::DynamicsFirstOrder(double _tau) : Dynamics(), tau(_tau) {}


const std::string ThrusterDynamicsYoerger::IDENTIFIER = "Yoerger";
REGISTER_DYNAMICS_CREATOR(ThrusterDynamicsYoerger, &ThrusterDynamicsYoerger::create)


Dynamics* ThrusterDynamicsYoerger::create(sdf::ElementPtr _sdf) {
  
  if (!_sdf->HasElement("alpha")) {
    std::cerr << "ThrusterDynamicsYoerger: expected element alpha" << std::endl;
    return NULL;
  }
  ...
  if (!_sdf->HasElement("beta")) {
    std::cerr << "ThrusterDynamicsYoerger: expected element beta" << std::endl;
    return NULL;
  }
...
ThrusterDynamicsYoerger::ThrusterDynamicsYoerger(double _alpha, double _beta) : Dynamics(), alpha(_alpha), beta(_beta) {}



const std::string ThrusterDynamicsBessa::IDENTIFIER = "Bessa";
REGISTER_DYNAMICS_CREATOR(ThrusterDynamicsBessa, &ThrusterDynamicsBessa::create)

Dynamics* ThrusterDynamicsBessa::create(sdf::ElementPtr _sdf) {
  
  if (!_sdf->HasElement("Jmsp")) {
    std::cerr << "ThrusterDynamicsBessa: expected element Jmsp" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("Kv1")) {
    std::cerr << "ThrusterDynamicsBessa: expected element Kv1" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("Kv2")) {
    std::cerr << "ThrusterDynamicsBessa: expected element Kv2" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("Kt")) {
    std::cerr << "ThrusterDynamicsBessa: expected element Kt" << std::endl;
    return NULL;
  }

  if (!_sdf->HasElement("Rm")) {
    std::cerr << "ThrusterDynamicsBessa: expected element Rm" << std::endl;
    return NULL;
  }
...
double ThrusterDynamicsBessa::update(double _cmd, double _t) {
  ...
  double dt = _t - prevTime;
  state += dt*(_cmd*Kt/Rm - Kv1*state - Kv2*state*std::abs(state))/Jmsp;

  return state;
}


ThrusterDynamicsBessa::ThrusterDynamicsBessa(double _Jmsp, double _Kv1, double _Kv2, double _Kt, double _Rm) : Dynamics(), Jmsp(_Jmsp), Kv1(_Kv1), Kv2(_Kv2), Kt(_Kt), Rm(_Rm) {}
