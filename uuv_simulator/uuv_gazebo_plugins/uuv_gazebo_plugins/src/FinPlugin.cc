...
void FinPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->dynamics.reset(DynamicsFactory::GetInstance().CreateDynamics(_sdf->GetElement("dynamics")));

  // Lift and drag model
  GZ_ASSERT(_sdf->HasElement("liftdrag"), "Could not find liftdrag");
  this->liftdrag.reset(LiftDragFactory::GetInstance().CreateLiftDrag(_sdf->GetElement("liftdrag")));

  // Subscribe to current velocity topic
  GZ_ASSERT(_sdf->HasElement("current_velocity_topic"),"Could not find current_velocity_topic.");
  std::string currentVelocityTopic = _sdf->Get<std::string>("current_velocity_topic");

  GZ_ASSERT(!currentVelocityTopic.empty(), "Fluid velocity topic tag cannot be empty");

  gzmsg << "Subscribing to current velocity topic: " << currentVelocityTopic << std::endl;
  this->currentSubscriber = this->node->Subscribe(currentVelocityTopic, &FinPlugin::UpdateCurrentVelocity, this);

  // Advertise the output topic
  this->anglePublisher = this->node->Advertise<uuv_gazebo_plugins_msgs::msgs::Double>(outputTopic);


  // Subscribe to the input signal topic
  this->commandSubscriber = this->node->Subscribe(inputTopic, &FinPlugin::UpdateInput, this);

  // Connect the update event
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&FinPlugin::OnUpdate, this, _1));
}
...
void FinPlugin::OnUpdate(const common::UpdateInfo &_info) {
  GZ_ASSERT(!std::isnan(this->inputCommand), "nan in this->inputCommand");
  ...
  this->angle = this->dynamics->update(this->inputCommand, _info.simTime.Double());
  ...
  ignition::math::Vector3d ldNormalI = finPose.Rot().RotateVector(ignition::math::Vector3d::UnitZ);
