...
void UnderwaterObjectPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("flow_velocity_topic")) {
    ...
    gzmsg << "Subscribing to current velocity topic: " << flowTopic << std::endl;
    this->flowSubscriber = this->node->Subscribe(flowTopic, &UnderwaterObjectPlugin::UpdateFlowVelocity, this);
  }
  ...
  if (_sdf->HasElement("link")) {
    for (sdf::ElementPtr linkElem = _sdf->GetElement("link"); linkElem; linkElem = linkElem->GetNextElement("link")) {
      ...
      hydro.reset(HydrodynamicModelFactory::GetInstance().CreateHydrodynamicModel(linkElem, link));
...
void UnderwaterObjectPlugin::InitDebug(physics::LinkPtr _link, HydrodynamicModelPtr _hydro)
{
...
void UnderwaterObjectPlugin::Update(const common::UpdateInfo &_info) {
  ...
  for (std::map<gazebo::physics::LinkPtr, HydrodynamicModelPtr>::iterator it = models.begin(); it != models.end(); ++it) {
    ...
    GZ_ASSERT(!std::isnan(linearAccel) && !std::isnan(angularAccel), "Linear or angular accelerations are invalid.");
...
void UnderwaterObjectPlugin::Connect() {
  ...
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&UnderwaterObjectPlugin::Update, this, _1));
}
...
void UnderwaterObjectPlugin::PublishRestoringForce( physics::LinkPtr _link) {
...
void UnderwaterObjectPlugin::PublishHydrodynamicWrenches(physics::LinkPtr _link) {
...
void UnderwaterObjectPlugin::GenWrenchMsg(ignition::math::Vector3d _force, ignition::math::Vector3d _torque, gazebo::msgs::WrenchStamped &_output) {
  common::Time curTime;
