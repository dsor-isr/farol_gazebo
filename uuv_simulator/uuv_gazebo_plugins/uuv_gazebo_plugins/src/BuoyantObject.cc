...
void BuoyantObject::GetBuoyancyForce(const ignition::math::Pose3d &_pose, ignition::math::Vector3d &buoyancyForce, ignition::math::Vector3d &buoyancyTorque) {
  ...
  if (!this->isSurfaceVessel) {
    ...
    if (!this->neutrallyBuoyant || volume != this->volume)
      ...
      buoyancyForce = ignition::math::Vector3d(0, 0, mass * this->g);
  } else {
    ...
    if (this->waterLevelPlaneArea <= 0) {
      this->waterLevelPlaneArea = this->boundingBox.XLength() * this->boundingBox.YLength();
      gzmsg << this->link->GetName() << "::" << "waterLevelPlaneArea = " << this->waterLevelPlaneArea << std::endl;
    }

    this->waterLevelPlaneArea = mass / (this->fluidDensity * this->submergedHeight);
    double curSubmergedHeight;
    GZ_ASSERT(this->waterLevelPlaneArea > 0.0, "Water level plane area must be greater than zero");
...
void BuoyantObject::SetBoundingBox(const AxisAlignedBox &_bBox) {
  ...
  gzmsg << "New bounding box for " << this->link->GetName() << "::" << this->boundingBox << std::endl;
}
...
void BuoyantObject::SetCoB(const ignition::math::Vector3d &_centerOfBuoyancy) {
  this->centerOfBuoyancy = ignition::math::Vector3d(_centerOfBuoyancy.X(), _centerOfBuoyancy.Y(), _centerOfBuoyancy.Z());
}
