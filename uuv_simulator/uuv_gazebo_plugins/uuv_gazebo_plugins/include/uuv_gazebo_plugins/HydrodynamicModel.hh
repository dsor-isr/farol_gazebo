...
class HydrodynamicModel : public BuoyantObject {
  ...
  public: virtual void ApplyHydrodynamicForces(double time, const ignition::math::Vector3d &_flowVelWorld) = 0;

  /// \brief Prints parameters
  public: virtual void Print(std::string _paramName, std::string _message = std::string()) = 0;

  /// \brief Return paramater in vector form for the given tag
  public: virtual bool GetParam(std::string _tag, std::vector<double>& _output) = 0;

  /// \brief Return paramater in vector form for the given tag
  public: virtual bool GetParam(std::string _tag, double& _output) = 0;
  ...
  protected: void ComputeAcc(Eigen::Vector6d _velRel, double _time, double _alpha = 0.3);
...
class HydrodynamicModelFactory {
  ...
  public: HydrodynamicModel* CreateHydrodynamicModel(sdf::ElementPtr _sdf, physics::LinkPtr _link);
  ...
  public: bool RegisterCreator(const std::string& _identifier, HydrodynamicModelCreator _creator);
...
#define REGISTER_HYDRODYNAMICMODEL_CREATOR(type, creator) const bool type::registeredWithFactory = HydrodynamicModelFactory::GetInstance().RegisterCreator(type::IDENTIFIER, creator);
...
class HMFossen : public HydrodynamicModel {
  ...
  public: static HydrodynamicModel* create(sdf::ElementPtr _sdf, physics::LinkPtr _link);
  ...
  /// \brief Prints parameters
  public: virtual void Print(std::string _paramName, std::string _message = std::string());
